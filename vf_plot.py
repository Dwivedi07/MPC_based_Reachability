...
# ----------------------------
models = []
for stage in range(1, num_stages):  # stages 1 to (current-1)
    path = f'model_checkpoints/stage_{stage}_best.pt'
    model = SingleBVPNet(
        in_features=4,
        out_features=1,
        hidden_features=512,
...
with torch.no_grad():
    V_hat = compute_recursive_value(x_tensor, t_tensor, dynamics, models)

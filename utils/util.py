...
##########################################################
########### Coupled versiono of Dataset Generation
##########################################################
...
########################################################## 
#### Decoupled version of the dataset generation
##########################################################

def compute_terminal_value(x, t, dynamics, models):
    """
    Computes the terminal value function V(x, t) using the model trained on the previous stage.
    
    Args:
        x: Tensor of shape [N, 3] (z, vz, K)
        t: Tensor of shape [N, 1] (times at which to evaluate V)
        dynamics: includes compute_l(x) and T_terminals {stage: torch.tensor([T])}
        models: trained models for the previous stage which will directly predict the terminal value
    Returns:
        V: Tensor of shape [N, 1] containing value function estimates at (x, t)
        
    V = l_x + (T_teminal_firststage - t) * Prev_stage_model(x, t)
    """
    l_x = dynamics.compute_l(x)
    T_terminals = dynamics.T_terminals
    if l_x.dim() == 1:
        l_x = l_x.unsqueeze(1)
    model = models[-1]  # take the last model
    T_f = T_terminals[1].to(x.device).expand(x.shape[0], 1) # final terminal time
    input_tensor = torch.cat([x, t], dim=1) # O_this stage eval at terminal of prev state
    with torch.no_grad():
        O_pred = model(input_tensor)
    
    delta_T = (T_f - t)  # [N, 1]
    V = l_x + delta_T * O_pred  # all [N, 1]
    
    return V  # shape: [batch_size, 1]



def generate_dataset_multi_decoupled(dynamics, size, N, R, H, u_std, stage, device, prev_stage_models = None, 
                    return_trajectories=False, use_grid_sampling=False):
    """
    Parallelized version of generate_dataset using GPU tensor ops.
    Args:
    ...
    for n in tqdm(range(size), desc="D_MPC samples", ncols=80, unit="sample"):
        ...
        for _ in range(R):
            ...
            if stage != 1:
                ...
                assert t_terminal.shape[0] == N
                assert t_terminal.shape[1] == 1 
                # just one call from previous stage model
                V_at_terminal_state = compute_terminal_value(x_terminal, t_terminal, dynamics, prev_stage_models)
                J_n = torch.minimum(V_at_terminal_state.reshape(-1), min_l_x_h)  # [N] : min( min(l), V_theta)
            else:
                J_n = min_l_x_h  # fallback if no previous model
